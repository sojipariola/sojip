  ...
  async function run() {
    ...
    try {
      const res = await apiFetch<GateResult>(
        `/projects/${projectId}/gates/${gate}/validate`,
        { method: "POST" }
      );
      setResult(res);
      onResult?.(res);
    } catch (e) {
      // 501 = gate not implemented yet for this phase
      if (
        e &&
        typeof e === "object" &&
        "status" in e &&
        ...
        setResult({
          gate,
          passed: false,
          reason: "Gate not yet implemented",
          missing: [`The ${gate} gate is coming soon.`],
          ai_feedback: null,
        });
      } else {
        setError(
      ...
      {!result && !loading && !error && (
        <>
          <p className="text-sm text-slate-500 mb-4 leading-relaxed">
            {description || DEFAULT_DESCRIPTIONS[gate]}
          </p>
          <button
            onClick={run}
      ...
      {loading && (
        <div className="space-y-3">
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            20–60 seconds on CPU inference. Under 5 seconds on GPU.
          </p>
